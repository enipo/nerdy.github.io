...
function draw() {
  ...
  doVolume();
  speedUp += 0.001;
  const sinus = sin(counter * 0.1) * ((levelAverage.mid) * 3);
  const cosinus = sin(counter * 0.1) * ((levelAverage.low) * 2) * getLevel().low;

  if (counter === 1) {
    viewObj.set(0.7, -900, 700, -4300);
  } else {
  ...
  const blurChance = 0.65 * random(1, 1 + cos(counter * 0.4));
  if (getLevel().all > blurChance || alwaysCleanBackground) {
    background(0);
    speedStuck = 0;
  ...
  const lowLevelMaxCalc = getLevel().low > levelMax.low;
  ...
  disc.setBaseColor(floor(noise(angle) * (255 * getLevel().mid * 3)));
  disc.setLevels(getLevel().low, getLevel().mid, getLevel().high);
  disc.show();

  const cosinusRatio = cos(counter * 0.06);
  for (var x = 0; x < round(pixelate.width * cosinusRatio); x++) {
    for (var y = 0; y < pixelate.height; y++) {
      ...
      if (brightnessPixel > 0.9) {
        ...
        const tz = -400;
        translate(tx - 100, ty + (1000 * sin(counter * 0.04)), tz);
  ...
  debug({
    ...
    levelAll: Number(getLevel().all).toFixed(decimals),
    levelMid: Number(getLevel().mid).toFixed(decimals),
    levelHigh: Number(getLevel().high).toFixed(decimals),
    levelLow: Number(getLevel().low).toFixed(decimals),
